...
func kernelVersion() (int, int, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("error parsing major version %q in %q: %v", s[0], sb.String(), err)
	}

	minor, err := strconv.Atoi(s[1])
	if err != nil {
		return 0, 0, fmt.Errorf("error parsing minor version %q in %q: %v", s[1], sb.String(), err)
	}
