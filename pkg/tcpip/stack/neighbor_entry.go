...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	case Incomplete:
		...
		sendMulticastProbe = func() {
			...
			retryCounter++
			e.timer = e.nic.stack.clock.NewTimer(&e.mu, sendMulticastProbe)
			e.timer.Reset(config.RetransmitTimer)
		}

		sendMulticastProbe()

	case Reachable:
		e.timer = e.nic.stack.clock.NewTimer(&e.mu, func() {
			e.dispatchChangeEventLocked(Stale)
			e.setStateLocked(Stale)
		})
		e.timer.Reset(e.nudState.ReachableTime())
	...
	case Delay:
		...
		e.timer.Reset(config.DelayFirstProbeTime)
	...
	case Probe:
		...
		sendUnicastProbe = func() {
			...
			e.timer = e.nic.stack.clock.NewTimer(&e.mu, sendUnicastProbe)
			e.timer.Reset(config.RetransmitTimer)
		}
	...
	case Failed:
		...
		e.timer.Reset(config.UnreachableTime)
