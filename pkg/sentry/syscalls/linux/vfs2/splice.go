...
func Sendfile(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	// Verify that infile is regular or block device. This is a
	...
	if stat, err := inFile.Stat(t, vfs.StatOptions{Mask: linux.STATX_TYPE}); err != nil {
		...
		(stat.Mode&linux.S_IFMT != linux.S_IFREG && stat.Mode&linux.S_IFBLK != linux.S_IFBLK) {
		return 0, nil, syserror.EINVAL
	}
	...
	if outIsPipe {
		...
		for n < count {
			...
			// multiple write calls to complete. Following Linux,
			// we always block on the write end.
			wbuf := buf[:n]
			for len(wbuf) > 0 {
				var writeN int64
