...
func TestDiscardsUDPPacketsWithMcastSourceAddressV4(t *testing.T) {
	...
	for _, mcastAddr := range []net.IP{
		...
		t.Run(fmt.Sprintf("srcaddr=%s", mcastAddr), func(t *testing.T) {
			conn.SendIP(
				testbench.IPv4{SrcAddr: testbench.Address(tcpip.Address(mcastAddr))},
				testbench.UDP{},
			)

			// Sleep for a second for the packet to go through.
			time.Sleep(time.Second)
			dut.SetNonBlocking(remoteFD, true)
			ret, payload, err := dut.RecvWithErrno(context.Background(), remoteFD, 100, 0)
			if err != syscall.EAGAIN || err != syscall.EWOULDBLOCK {
				t.Errorf("Recv got unexpected result, ret=%d, payload=%q, err=%s", ret, payload, err)
			}
		})
	}
}
...
func TestDiscardsUDPPacketsWithMcastSourceAddressV6(t *testing.T) {
	...
	for _, mcastAddr := range []net.IP{
		...
		t.Run(fmt.Sprintf("srcaddr=%s", mcastAddr), func(t *testing.T) {
			conn.SendIPv6(
				testbench.IPv6{SrcAddr: testbench.Address(tcpip.Address(mcastAddr))},
				testbench.UDP{},
			)
