...
TEST_P(SocketInetLoopbackTest, TCPPassiveCloseNoTimeWaitTest_NoRandomSave) {
  ...
  // The sleeps are required to ensure that it is conn_fd that moves to
  // CLOSE-WAIT and not accepted. Without the sleep there is a chance of a race
  // and the test can become flaky.
  absl::SleepFor(absl::Seconds(1));
  conn_fd.reset();
  absl::SleepFor(absl::Seconds(1));
...
TEST_P(SocketInetLoopbackTest, TCPActiveCloseTimeWaitTest_NoRandomSave) {
  ...
  ASSERT_THAT(shutdown(conn_fd.get(), SHUT_RDWR), SyscallSucceeds());
  // The sleeps are required to ensure that it is conn_fd that moves to
  // TIME-WAIT and not accepted. Without the sleep there is a chance of a race
  // and the test can become flaky.
  absl::SleepFor(absl::Seconds(1));
  accepted.reset();
  absl::SleepFor(absl::Seconds(1));

  // Now close the conn_fd. This should not cause TIME-WAIT to be terminated
  // prematurely.
  conn_fd.reset();
